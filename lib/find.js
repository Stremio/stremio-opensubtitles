...
client.options.headers['User-Agent'] = 'Stremio-addon-fetch'
...
function pickSubtitles(query, res, subtitles) 
{
	if (! Array.isArray(res)) {
		subtitles.all = Array.isArray(subtitles.all) ? subtitles.all : []
		return subtitles
	}
    ...
    subtitles.all = res.map(function(x) { 
    	return { 
			id: x.IDSubtitle, 
			url: modifyDownloadUrl(x.SubDownloadLink),
...
function modifyDownloadUrl(url) {
	...
	return url
		.replace('download/src-api', 'download/subencoding-utf8/src-api')
		.replace(/\.gz$/, '')
...
function findSubtitles(args, callback)
{
	...
	if (args.query) {
		...
		if (args.query.itemHash) {
			var split = args.query.itemHash.split(" ")
			query.meta = split.length > 1
				? { imdb_id: split[0], season: parseInt(split[1]), episode: parseInt(split[2]) }
				: { imdb_id: split[0] }
		}
	...
	*/
	...
	var hashQuery = query.movieHash ? {
			moviebytesize: parseInt(query.movieByteSize),
			moviehash: query.movieHash,
			sublanguageid: null
	} : null;
	
	var metaQuery = (query.meta && typeof(query.meta.imdb_id) === 'string' && query.meta.imdb_id.indexOf('tt') === 0) ? {
		imdbid: query.meta.imdb_id.slice(2),
		sublanguageid: null
	} : null;

	if (!hashQuery && !metaQuery) {
		subtitles.all = []
	...
	async.auto({
		...
		retrieve: ["token", function(cb, res) {
			...
			var queries = [hashQuery, metaQuery].filter(function(x) { return x })
			opensubRequest("SearchSubtitles", [res.token, queries, { limit: 100 } ], cb);
		}],
