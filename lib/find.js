...
function pickSubtitles(query, res, subtitles) 
{
    ...
    subtitles.all = res.map(function(x) { 
    	return { 
			id: x.IDSubtitle, 
			url: x.SubDownloadLink,
			SubEncoding: x.SubEncoding,
	...
	*/
	...
	async.auto({
		...
		retrieve: ["token", function(cb, res) {
			...
			opensubRequest("SearchSubtitles", [res.token, queries ], cb);
		}],
		pick: ["retrieve", function(cb, res) {
			...
			pickSubtitles(query, res.retrieve.data, subtitles, { limit: 400 });
			cb();
		}] 
